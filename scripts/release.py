...
VERSION_FILE = REPO_ROOT / "cocso_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+cocso-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "cocso@nousresearch.com": "NousResearch",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "cocso@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url=None,
    ...
    from cocso_cli.branding import default_branding as _gen_branding
    lines.append(f"# {_gen_branding('agent_name', 'COCSO Agent')} v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for COCSO Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="COCSO Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  COCSO Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"COCSO Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"COCSO Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'COCSO Agent v{new_version} ({calver_date})' "
