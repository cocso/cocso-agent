#!/usr/bin/env bash
# Canonical test runner for cocso-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.cocso/cocso-agent/venv"; do
...
# Unset COCSO_* behavioral vars too.
unset COCSO_YOLO_MODE COCSO_INTERACTIVE COCSO_QUIET COCSO_TOOL_PROGRESS \
      COCSO_TOOL_PROGRESS_MODE COCSO_MAX_ITERATIONS COCSO_SESSION_PLATFORM \
      COCSO_SESSION_CHAT_ID COCSO_SESSION_CHAT_NAME COCSO_SESSION_THREAD_ID \
      COCSO_SESSION_SOURCE COCSO_SESSION_KEY COCSO_GATEWAY_SESSION \
      COCSO_PLATFORM COCSO_INFERENCE_PROVIDER COCSO_MANAGED COCSO_DEV \
      COCSO_CONTAINER COCSO_EPHEMERAL_SYSTEM_PROMPT COCSO_TIMEZONE \
      COCSO_REDACT_SECRETS COCSO_BACKGROUND_NOTIFICATIONS COCSO_EXEC_ASK \
      COCSO_HOME_MODE 2>/dev/null || true
