...
# Configuration
REPO_URL_SSH="git@github.com:cocso/cocso-agent.git"
REPO_URL_HTTPS="https://github.com/cocso/cocso-agent.git"
COCSO_HOME="${COCSO_HOME:-$HOME/.cocso}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${COCSO_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$COCSO_INSTALL_DIR"
...
#   code at /usr/local/lib/cocso-agent, command at /usr/local/bin/cocso,
#   data still at /root/.cocso (COCSO_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.cocso/cocso-agent"
            echo "                   default (root, Linux): /usr/local/lib/cocso-agent"
            echo "  --cocso-home PATH  Data directory (default: ~/.cocso, or \$COCSO_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, COCSO installs the code under"
            echo "  /usr/local/lib/cocso-agent and links the command into"
            echo "  /usr/local/bin/cocso (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$COCSO_HOME"
            echo "  (default /root/.cocso).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$COCSO_HOME/cocso-agent are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `cocso` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $COCSO_HOME/cocso-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $COCSO_HOME/cocso-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/cocso-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $COCSO_HOME/cocso-agent — then preserve it)
#
# Always no-op when the user set --dir or $COCSO_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in COCSO_HOME.
    if is_termux; then
        INSTALL_DIR="$COCSO_HOME/cocso-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$COCSO_HOME/cocso-agent/.git" ]; then
            INSTALL_DIR="$COCSO_HOME/cocso-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/cocso-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/cocso-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/cocso"
        log_info "  Data:    $COCSO_HOME (unchanged)"
        return 0
    fi
...
get_cocso_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/cocso" ]; then
        echo "$link_dir/cocso"
    else
        echo "cocso"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/cocso/cocso-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$COCSO_HOME/node/bin/node" ]; then
        export PATH="$COCSO_HOME/node/bin:$PATH"
        local found_ver=$("$COCSO_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (COCSO-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.cocso/node/..."
    ...
    # Place into ~/.cocso/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$COCSO_HOME/node"
    mkdir -p "$COCSO_HOME"
    mv "$extracted_dir" "$COCSO_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$COCSO_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$COCSO_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$COCSO_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$COCSO_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$COCSO_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.cocso/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "COCSO Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="cocso-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if COCSO behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up cocso command..."

    if [ "$USE_VENV" = true ]; then
        COCSO_BIN="$INSTALL_DIR/venv/bin/cocso"
    else
        COCSO_BIN="$(which cocso 2>/dev/null || echo "")"
        if [ -z "$COCSO_BIN" ]; then
            log_warn "cocso not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$COCSO_BIN" ]; then
        log_warn "cocso entry point not found at $COCSO_BIN"
    ...
    # Create a user-facing shim for the cocso command.
    mkdir -p "$command_link_dir"
    ln -sf "$COCSO_BIN" "$command_link_dir/cocso"
    log_success "Symlinked cocso → $command_link_display_dir/cocso"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "cocso command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v cocso' \
                >/dev/null 2>&1; then
        ...
        log_info "cocso not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# COCSO Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# COCSO Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# COCSO Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so cocso works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.cocso directory structure (config at top level, code in subdir)
    mkdir -p "$COCSO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.cocso/.env (top level, easy to find)
    if [ ! -f "$COCSO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$COCSO_HOME/.env"
            log_success "Created ~/.cocso/.env from template"
        else
            touch "$COCSO_HOME/.env"
            log_success "Created ~/.cocso/.env"
        fi
    else
        log_info "~/.cocso/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.cocso/config.yaml (top level, easy to find)
    if [ ! -f "$COCSO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$COCSO_HOME/config.yaml"
            log_success "Created ~/.cocso/config.yaml from template"
        fi
    else
        log_info "~/.cocso/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$COCSO_HOME/SOUL.md" ]; then
        cat > "$COCSO_HOME/SOUL.md" << 'SOUL_EOF'
# COCSO Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how COCSO communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.cocso/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.cocso/"

    # Seed bundled skills into ~/.cocso/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.cocso/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.cocso/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$COCSO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$COCSO_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.cocso/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — COCSO itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (cocso --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'cocso setup' after install."
        return 0
    fi
    ...
    # Run cocso setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m cocso_cli.main setup < /dev/tty
    else
        python -m cocso_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$COCSO_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for COCSO to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$COCSO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'cocso whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                COCSO_CMD="$(get_cocso_command_path)"
                $COCSO_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'cocso whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'cocso gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $COCSO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $COCSO_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: cocso gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: cocso gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $COCSO_CMD gateway > "$COCSO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.cocso/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: cocso gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: cocso gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $COCSO_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $COCSO_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $COCSO_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}cocso${NC}              Start chatting"
    echo -e "   ${GREEN}cocso setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}cocso config${NC}       View/edit configuration"
    echo -e "   ${GREEN}cocso config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}cocso gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}cocso update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'cocso' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'cocso' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'cocso' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
