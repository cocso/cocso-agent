# ============================================================================
# COCSO Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$COCSOHome = "$env:LOCALAPPDATA\cocso",
    [string]$InstallDir = "$env:LOCALAPPDATA\cocso\cocso-agent"
)
...
$RepoUrlSsh = "git@github.com:cocso/cocso-agent.git"
$RepoUrlHttps = "https://github.com/cocso/cocso-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             🅒 COCSO Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
    Write-Host "│  COCSO Agent.                                              │" -ForegroundColor Magenta
    Write-Host "└─────────────────────────────────────────────────────────┘" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Node {
    ...
    $managedNode = "$COCSOHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$COCSOHome\node;$env:Path"
        Write-Success "Node.js $version found (COCSO-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.cocso/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\cocso-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$COCSOHome\node") { Remove-Item -Recurse -Force "$COCSOHome\node" }
                Move-Item $extractedDir.FullName "$COCSOHome\node"
                $env:Path = "$COCSOHome\node;$env:Path"

                $version = & "$COCSOHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.cocso/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/cocso/cocso-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\cocso-agent-$Branch.zip"
                $extractPath = "$env:TEMP\cocso-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up cocso command..."
    
    if ($NoVenv) {
        $cocsoBin = "$InstallDir"
    } else {
        $cocsoBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so cocso is globally available
    # On Windows, the cocso.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$cocsoBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$cocsoBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $cocsoBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set COCSO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\cocso instead
    # of the Unix default ~/.cocso
    $currentCOCSOHome = [Environment]::GetEnvironmentVariable("COCSO_HOME", "User")
    if (-not $currentCOCSOHome -or $currentCOCSOHome -ne $COCSOHome) {
        [Environment]::SetEnvironmentVariable("COCSO_HOME", $COCSOHome, "User")
        Write-Success "Set COCSO_HOME=$COCSOHome"
    }
    $env:COCSO_HOME = $COCSOHome
    
    # Update current session
    $env:Path = "$cocsoBin;$env:Path"
    
    Write-Success "cocso command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.cocso directory structure
    New-Item -ItemType Directory -Force -Path "$COCSOHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$COCSOHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$COCSOHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$COCSOHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$COCSOHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$COCSOHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$COCSOHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$COCSOHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$COCSOHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$COCSOHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.cocso/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.cocso/.env"
        }
    } else {
        Write-Info "~/.cocso/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$COCSOHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.cocso/config.yaml from template"
        }
    } else {
        Write-Info "~/.cocso/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$COCSOHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.cocso/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.cocso/"
    
    # Seed bundled skills into ~/.cocso/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.cocso/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.cocso/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$COCSOHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.cocso/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        try {
            ...
            Write-Warn "TUI npm install failed (cocso --tui may not work)"
        }
        Pop-Location
    }
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m cocso_cli.main setup
    } else {
        python -m cocso_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $cocsoCmd = "$InstallDir\venv\Scripts\cocso.exe"
    if (-not (Test-Path $cocsoCmd)) {
        $cocsoCmd = "cocso"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$COCSOHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'cocso whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$COCSOHome\logs\gateway.log"
            Start-Process -FilePath $cocsoCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$COCSOHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: cocso gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: cocso gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$COCSOHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$COCSOHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$COCSOHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$COCSOHome\cocso-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   cocso              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   cocso setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   cocso config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   cocso config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   cocso gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   cocso update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/cocso/cocso-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
