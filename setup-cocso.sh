...
# 5. Symlinks the 'cocso' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}🪽 COCSO Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink cocso into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up cocso command..."

COCSO_BIN="$SCRIPT_DIR/venv/bin/cocso"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$COCSO_BIN" "$COMMAND_LINK_DIR/cocso"
echo -e "${GREEN}✓${NC} Symlinked cocso → $COMMAND_LINK_DISPLAY_DIR/cocso"
...
# ============================================================================
# Seed bundled skills into ~/.cocso/skills/
# ============================================================================

COCSO_SKILLS_DIR="${COCSO_HOME:-$HOME/.cocso}/skills"
mkdir -p "$COCSO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.cocso/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$COCSO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  cocso status        # Check configuration"
if is_termux; then
    echo "  cocso gateway       # Run gateway in foreground"
else
    echo "  cocso gateway install # Install gateway service (messaging + cron)"
fi
echo "  cocso cron list     # View scheduled jobs"
echo "  cocso doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m cocso_cli.main setup
fi
