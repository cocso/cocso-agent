...
Usage:
    from cocso_core.toolsets import get_toolset, resolve_toolset, get_all_toolsets
...
_COCSO_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (COCSO_KANBAN_TASK env set), otherwise
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (COCSO_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full COCSO toolsets (CLI + messaging platforms)
    #
    ...
    "cocso-cli": {
        ...
        "tools": _COCSO_CORE_TOOLS,
        "includes": []
    },

    "cocso-cron": {
        # Mirrors cocso-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `cocso tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as cocso-cli; gated by `cocso tools`",
        "tools": _COCSO_CORE_TOOLS,
        "includes": []
    },
    ...
    "cocso-gateway": {
        ...
        "tools": [],
        "includes": ["cocso-telegram", "cocso-discord", "cocso-slack", "cocso-webhook"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (cocso-<name>).
        # Gives them _COCSO_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("cocso-"):
            platform_name = name[len("cocso-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_COCSO_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
