"""
Unified tool configuration for COCSO Agent.

`cocso tools` and `cocso setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.cocso/config.yaml under
...
from cocso_cli.branding import BRAND_EMOJI, DEFAULT_REPO_HTTPS_URL
...
from cocso_cli.cli_output import (  # noqa: E402 — late import block
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("browser",         "🌐 Browser",                  "open, click, type"),
    ("terminal",        "💻 Terminal",                 "processes and shell execution"),
    ("file",            "📁 Files",                    "read, write, patch, search"),
    ("vision",          "👁️ Vision",                   "image analysis"),
    ("todo",            "📋 Planning",                 "task planning"),
    ("memory",          "💾 Memory",                   "persistent memory"),
    ("messaging",       "📨 Messaging",                "send messages across connected platforms"),
    ("discord",         "💬 Discord",                  "read and participate in Discord"),
    ("discord_admin",   "🛡️ Discord Admin",            "server admin operations"),
]

# Toolsets that are OFF by default for new installs.
# They're still in _COCSO_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"browser", "discord", "discord_admin"}

# Platform-scoped toolsets: only appear in the `cocso tools` checklist for
...
def _get_effective_configurable_toolsets():
    """Return the intentionally small COCSO toolset checklist."""
    return list(CONFIGURABLE_TOOLSETS)


def _get_plugin_toolset_keys() -> set:
    """COCSO does not expose plugin toolsets in the default checklist."""
    return set()

# Platform display config — derived from the canonical registry so every
# module shares the same data.  Kept as dict-of-dicts for backward
# compatibility with existing ``PLATFORMS[key]["label"]`` access patterns.
from cocso_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_cocso_home()}/cocso-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in cocso-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/cocso-agent:latest"
            )
            return
        ...
        # Run the full `cocso auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.cocso/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from cocso_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: cocso auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `cocso auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: cocso auth spotify")
        except Exception as exc:
        ...
        try:
            from cocso_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: cocso plugins enable observability/langfuse")
        _print_info("    Restart COCSO for tracing to take effect.")
        _print_info("    Verify: cocso plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    from cocso_core.toolsets import resolve_toolset, TOOLSETS
    ...
    # "cocso-cli" (which include all _COCSO_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "cocso-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # otherwise saving via `cocso tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"cocso-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("cocso-")}
    ...
    # shipped as a bundled plugin but user must opt in via `cocso tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `cocso tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by cocso tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (cocso-cli, cocso-telegram, etc.)
    ...
    # Opening `cocso tools` is the user's opt-in to reconfigure tools, so treat
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    try:
        ...
        from cocso_core import model_tools  # noqa: F401
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str], platform: str = "cli") -> Set[str]:
    ...
    from cocso_core.toolsets import resolve_toolset
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `cocso tools` and `cocso setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color(f"{BRAND_EMOJI} COCSO Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from cocso_core.cocso_constants import display_cocso_home
    print(color(f"  Tool configuration saved to {display_cocso_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'cocso' or gateway restart.", Colors.DIM))
    print()
