"""
Profile management for multiple isolated COCSO instances.

Each profile is a fully independent COCSO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.cocso/profiles/<name>/`` by default.

The "default" profile is ``~/.cocso`` itself — backward compatible,
zero migration needed.

Usage::

    cocso profile create coder          # fresh profile + bundled skills
    cocso profile create coder --clone  # also copy config, .env, SOUL.md
    cocso profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    cocso -p coder chat                 # or via flag
    cocso profile use coder             # set as sticky default
    cocso profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See cocso_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.cocso`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.cocso) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "cocso-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "cocso", "default", "test", "tmp", "root", "sudo",
})

# COCSO subcommands that cannot be used as profile names/aliases
_COCSO_SUBCOMMANDS = frozenset({
    "chat", "model", "gateway", "setup", "logout",
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the cocso root, NOT to the current COCSO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where COCSO_HOME points outside
    ``~/.cocso``, profiles live under ``COCSO_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_cocso_home() / "profiles"


def _get_default_cocso_home() -> Path:
    """Return the default (pre-profile) COCSO_HOME path.

    In standard deployments this is ``~/.cocso``.
    In Docker/custom deployments where COCSO_HOME is outside ``~/.cocso``
    (e.g. ``/opt/data``), returns COCSO_HOME directly.
    """
    from cocso_core.cocso_constants import get_default_cocso_root
    return get_default_cocso_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_cocso_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.cocso
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its COCSO_HOME directory."""
    if name == "default":
        return _get_default_cocso_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, cocso subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _COCSO_SUBCOMMANDS:
        return f"'{name}' conflicts with a cocso subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_cocso_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.cocso)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.cocso/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    if not soul_path.exists():
        try:
            from cocso_cli.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches COCSO_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "COCSO_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.cocso).\n"
            "To remove everything, use: cocso uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set COCSO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("COCSO_HOME")
    try:
        os.environ["COCSO_HOME"] = str(profile_dir)
        from cocso_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["COCSO_HOME"] = old_home
        elif "COCSO_HOME" in os.environ:
            del os.environ["COCSO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.cocso/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: cocso profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from COCSO_HOME.

    Returns ``"default"`` if COCSO_HOME is not set or points to ``~/.cocso``.
    Returns the profile name if COCSO_HOME points into ``~/.cocso/profiles/<name>``.
    Returns ``"custom"`` if COCSO_HOME is set to an unrecognized path.
    """
    from cocso_core.cocso_constants import get_cocso_home
    cocso_home = get_cocso_home()
    resolved = cocso_home.resolve()

    default_resolved = _get_default_cocso_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.cocso itself — its parent is ~/ and its
        # directory name is ".cocso", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: cocso profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.cocso itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.cocso). "
            "Specify a different name: cocso profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="cocso_profile_import_") as tmpdir:
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for cocso profile names."""
    return '''# COCSO Agent profile completion
# Add to ~/.bashrc: eval "$(cocso completion bash)"

_cocso_profiles() {
    local profiles_dir="$HOME/.cocso/profiles"
...
_cocso_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_cocso_profiles)" -- "$cur"))
        return
    fi
...
complete -F _cocso_completion cocso
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for cocso profile names."""
    return '''#compdef cocso
# COCSO Agent profile completion
# Add to ~/.zshrc: eval "$(cocso completion zsh)"

_cocso() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.cocso/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.cocso/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a COCSO_HOME path string.

    Called early in the CLI entry point, before any cocso modules
    are imported, to set the COCSO_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: cocso profile create {profile_name}"
        )
