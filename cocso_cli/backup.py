"""
Backup and import commands for cocso CLI.

`cocso backup` creates a zip archive of the entire ~/.cocso/ directory
(excluding the cocso-agent repo and transient files).

`cocso import` restores from a backup zip, overlaying onto the current
COCSO_HOME root.
"""
...
from cocso_core.cocso_constants import get_default_cocso_root, get_cocso_home, display_cocso_home
...
_EXCLUDED_DIRS = {
    "cocso-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to cocso root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the COCSO home directory."""
    cocso_root = get_default_cocso_root()

    if not cocso_root.is_dir():
        print(f"Error: COCSO home directory not found at {cocso_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"cocso-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"cocso-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_cocso_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(cocso_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(cocso_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(cocso_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside cocso root
            try:
    ...
    print(f"\nRestore with: cocso import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a COCSO backup.
    ...
    # Look for telltale files that a cocso home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a COCSO backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.cocso/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a cocso dir name
        if prefix in (".cocso", "cocso"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_cocso_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (cocso_root / "config.yaml").exists()
        has_env = (cocso_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has COCSO configuration.")
        ...
        cocso_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = cocso_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(cocso_root.resolve())
            except ValueError:
        ...
        profiles_dir = cocso_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # cocso_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: cocso profile list  (after installing cocso)")

        # Guidance
        print()
        if not (cocso_root / "cocso-agent").is_dir():
            print("Note: The cocso-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: cocso update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  cocso -p {pname} gateway install")

        print("Done. Your COCSO configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and cocso backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to COCSO_HOME).
...
# — `cocso update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(cocso_home: Optional[Path] = None) -> Path:
    home = cocso_home or get_cocso_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    cocso_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(cocso_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(cocso_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for cocso backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_cocso_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, cocso_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``cocso_root`` to ``out_path``.
...
def _pre_update_backup_dir(cocso_home: Optional[Path] = None) -> Path:
    home = cocso_home or get_cocso_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of COCSO_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<COCSO_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``cocso update``) should continue even if the backup fails.
    """
    cocso_root = cocso_home or get_default_cocso_root()
    if not cocso_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(cocso_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, cocso_root)
    if result is None:
        return None
