#!/usr/bin/env python3
"""
COCSO Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the COCSO Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills cocso-agent-dev,github-auth
...
os.environ["COCSO_QUIET"] = "1"  # Our own modules
...
from cocso_cli.banner import _format_context_length, format_banner_version_label
...
from cocso_cli.browser_connect import (
    DEFAULT_BROWSER_CDP_URL,
    manual_chrome_debug_command,
    try_launch_chrome_debug,
)
from cocso_cli.env_loader import load_cocso_dotenv
from cocso_core.utils import base_url_host_matches

_cocso_home = get_cocso_home()
_project_env = Path(__file__).parent / '.env'
load_cocso_dotenv(cocso_home=_cocso_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.cocso/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If COCSO_IGNORE_USER_CONFIG=1 is set (via ``cocso chat --ignore-user-config``),
    the user config at ``~/.cocso/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({COCSO_HOME}/config.yaml)
    user_config_path = _cocso_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("COCSO_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain COCSO, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me COCSO - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and COCSOCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `cocso model`).
    ...
    # Normalize config key: the new config system (cocso_cli/config.py) and all
    ...
    env_mappings = {
        ...
        # Docker resource config — ignored for local/ssh.
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["COCSO_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.cocso/logs/.
...
try:
    from cocso_cli.skin_engine import init_skin_from_config
...
from cocso_core.model_tools import get_tool_definitions, get_toolset_for_tool

# Extracted CLI modules (Phase 3)
from cocso_cli.banner import build_welcome_banner
from cocso_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
from cocso_core.toolsets import get_all_toolsets, get_toolset_info, validate_toolset
...
from cocso_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run cocso -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"cocso-{short_id}"
    branch_name = f"cocso/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`cocso_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from cocso_core.cocso_constants import get_cocso_home as _get_cocso_home
        ...
        session_db.maybe_auto_prune_and_vacuum(
            ...
            sessions_dir=_get_cocso_home() / "sessions",
        )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`cocso_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``cocso/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("cocso-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``cocso/cocso-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``cocso -w`` sessions and PR review
    ...
    orphaned = [
        ...
        if b not in active_branches
           and (b.startswith("cocso/cocso-") or b.startswith("pr-"))
    ]
...
from cocso_core.cocso_constants import is_termux as _is_termux_environment
...
def _build_compact_banner() -> str:
    ...
    from cocso_cli.skin_engine import get_active_brand_emoji
    from cocso_cli.branding import default_branding
    _emoji = get_active_brand_emoji()
    _default_name = default_branding("agent_name", "COCSO Agent")
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from cocso_cli.plugins import get_plugin_manager
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.cocso/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.cocso/config.yaml on first use)
        ...
        from cocso_core.utils import atomic_yaml_write
...
class COCSOCLI:
    """
    Interactive CLI for the COCSO Agent.
    ...
    def __init__(
        ...
        """
        Initialize the COCSO CLI.

        Args:
            model: Model to use (default: from env or claude-sonnet)
            toolsets: List of toolsets to enable (default: all)
            provider: Inference provider ("auto", "anthropic", "openai", "openai-codex", "xiaomi", "lmstudio", "custom")
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from cocso_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
                ...
                or os.getenv("COCSO_INFERENCE_PROVIDER")
                or "auto"
        )
        ...
        elif os.getenv("COCSO_MAX_ITERATIONS"):
            self.max_turns = int(os.getenv("COCSO_MAX_ITERATIONS"))
        else:
            self.max_turns = 90
        ...
        # by `cocso chat --ignore-rules` in cocso_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("COCSO_IGNORE_RULES") == "1"

        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
                os.getenv("COCSO_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all COCSO processes for this COCSO_HOME.
        ...
        self._history_file = _cocso_home / ".cocso_history"
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            from cocso_cli.skin_engine import get_active_agent_short_name
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from cocso_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from cocso_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from cocso_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from cocso_cli.skin_engine import get_active_skin
                from cocso_cli.branding import default_branding, default_color
                _default_label = default_branding("response_label", " 🪽 COCSO ").strip()
                _skin = get_active_skin()
                ...
                from cocso_cli.branding import default_branding, default_color
                label = default_branding("response_label", " 🪽 COCSO ").strip()
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set ANTHROPIC_API_KEY / OPENAI_API_KEY or run: cocso setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: cocso setup")
            return False
        ...
        # `cocso chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `cocso auth add openai-codex`
        # without `cocso model`), fall back to the provider's first catalog
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None,
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (cocso sessions list).{_RST}")
                return False
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.cocso/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from cocso_cli.clipboard import save_clipboard_image

        img_dir = get_cocso_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: cocso --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for COCSO config/state.
        ...
        if subcmd in ("list", "ls"):
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_cocso_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from cocso_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(
                f"  {_DIM}Tip: type your next message, or run cocso chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            from cocso_core.model_tools import check_tool_availability
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'cocso setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        """
        from cocso_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from cocso_cli.skin_engine import get_active_help_header
        ...
        from cocso_cli.skin_engine import get_active_agent_short_name as _gsn
        chunks.append(
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in cocso_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from cocso_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("COCSO_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `cocso sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from cocso_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `cocso sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.cocso/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``cocso --resume <id>``
        ...
        saved_dir = get_cocso_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"cocso_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: cocso --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from cocso_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        # Codex OAuth-enforced caps win over the raw models.dev entry
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `cocso model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from cocso_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from cocso_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cocso_cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
        ...
        custom_provs = None
        try:
            from cocso_cli.config import get_compatible_custom_providers, load_config
            cfg = load_config()
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from cocso_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to cocso_cli.curator so the CLI and the `cocso curator`
        ...
        try:
            from cocso_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from cocso_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to cocso_cli.skills_hub."""
        from cocso_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_cocso_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_cocso_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in cocso_cli/commands.py instead of touching every dispatch site.
        from cocso_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in ("quit", "exit", "q"):
            ...
            from cocso_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_cocso_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from cocso_cli.commands import COMMANDS
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from cocso_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_cocso_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from cocso_cli.config import load_config
        from cocso_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = bool(os.environ.get("COCSO_YOLO_MODE"))
        if current:
            os.environ.pop("COCSO_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while COCSO is working.
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from cocso_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from cocso_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            for quiet_logger in ('tools', 'run_agent', 'trajectory_compressor', 'cron', 'cocso_cli'):
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from cocso_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.cocso/skills/ and queue a note for the
        next user turn.
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        if isinstance(message, str):
            from cocso_cli.run_agent import _sanitize_surrogates
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _cocso_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  cocso --resume {self.session_id}")
            if session_title:
                print(f"  cocso -c \"{session_title}\"")
            print()
            ...
            try:
                from cocso_cli.skin_engine import get_active_goodbye
                from cocso_cli.branding import default_branding
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from cocso_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from cocso_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        def handle_ctrl_z(event):
            ...
            from cocso_cli.skin_engine import get_active_skin
            from cocso_cli.branding import default_branding
            agent_name = get_active_skin().get_branding("agent_name", default_branding("agent_name", "COCSO Agent"))
        ...
        # Dynamic prompt: shows COCSO symbol when agent is working,
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = COCSOCLI._compute_model_picker_viewport(
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``COCSO_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("COCSO_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: cocso setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    COCSO Agent CLI - Interactive AI Assistant
    ...
    os.environ["COCSO_INTERACTIVE"] = "1"
    ...
    # Default to cocso-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside COCSOCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  COCSO_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "COCSO" box, no tool-gen
