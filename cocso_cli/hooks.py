"""cocso hooks — inspect and manage shell-script hooks.

Usage::

    cocso hooks list
    cocso hooks test <event> [--for-tool X] [--payload-file F]
    cocso hooks revoke <command>
    cocso hooks doctor

Consent records live under ``~/.cocso/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.cocso/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``cocso hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: cocso hooks {list|test|revoke|doctor}")
        print("Run 'cocso hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.cocso/config.yaml.")
        print("See `cocso hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `cocso hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `cocso hooks test` and `cocso hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (COCSO wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `cocso hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `cocso hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `cocso hooks doctor`.")
