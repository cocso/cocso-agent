"""
Configuration management for COCSO Agent.

Config files are stored in ~/.cocso/ for easy access:
- ~/.cocso/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.cocso/.env         - API keys and secrets

This module provides:
- cocso config          - Show current configuration
- cocso config edit     - Open config in editor
- cocso config set      - Set a specific value
- cocso config wizard   - Re-run setup wizard
"""
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("COCSO_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_cocso_home() / ".managed"
...
def is_managed() -> bool:
    """Check if COCSO is running in package-manager-managed mode.

    Two signals: the COCSO_MANAGED env var (set by the systemd service),
    or a .managed marker file in COCSO_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade cocso-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "cocso update"


def format_managed_message(action: str = "modify this COCSO installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("COCSO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this COCSO installation is managed by NixOS "
            f"(COCSO_MANAGED={env_hint}).\n"
            "Edit services.cocso-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this COCSO installation is managed by Homebrew "
            f"(COCSO_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade cocso-agent"
        )

    return (
        f"Cannot {action}: this COCSO installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall COCSO."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from COCSO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, cocso_bin
    or None if container mode is not active, we're already inside the
    container, or COCSO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("COCSO_DEV") == "1":
        return None
    ...
    container_mode_file = get_cocso_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "cocso-agent")
    exec_user = info.get("exec_user", "cocso")
    cocso_bin = info.get("cocso_bin", "/data/current-package/bin/cocso")
...
# Re-export from cocso_core.cocso_constants — canonical definition lives there.
from cocso_core.cocso_constants import get_cocso_home  # noqa: F811,E402
from cocso_core.utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_cocso_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_cocso_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the cocso group can
    share state with the gateway service.

    The mode can be overridden via the COCSO_HOME_MODE environment variable
    (e.g. COCSO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse COCSO_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("COCSO_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When COCSO runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("COCSO_CONTAINER") or os.environ.get("COCSO_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set COCSO_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into COCSO_HOME if the user doesn't have one yet.
    ...
    if soul_path.exists():
        return
    from cocso_cli.default_soul import render_default_soul
    user_name = ""
    try:
...
def ensure_cocso_home():
    """Ensure ~/.cocso directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_cocso_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_cocso_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"COCSO_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the COCSO-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, COCSO auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), COCSO sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when COCSO
        ...
        #  "/home/user/.cocso/cache/documents:/output"]
        ...
        # bundled COCSO image, which drops to the `cocso` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, COCSO sends a stable profile-scoped userId to Camofox
    ...
    "checkpoints": {
        ...
        # ~/.cocso/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, cocso sweeps at startup (at most once per
        # min_interval_hours) and deletes:
        #   * orphan repos: COCSO_WORKDIR no longer exists on disk
    ...
    # single read_file page exceeds these limits, COCSO truncates the
    ...
    "display": {
        ...
        # e.g. `model · 68% · ~/projects/cocso`. Per-platform overrides go under
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.cocso/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        ...
        # Add user-installed plugins under ~/.cocso/plugins/memory/<name>/
        # to activate them via this field.
        "provider": "",
    },
    ...
    # always goes to ~/.cocso/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${COCSO_SKILL_DIR} and ${COCSO_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `cocso curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # to use COCSO' main auxiliary client resolution.
        "auxiliary": {
            "provider": None,
            "model": None,
        },
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "🪽 *COCSO Agent*" header.
    ...
    # stored approval from ~/.cocso/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or COCSO_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via COCSO_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     cocso-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.cocso/logs/.
    ...
    # lists without shipping a cocso-agent release.  Empty by default;
    # set ``url`` to a JSON manifest you host yourself.
    "model_catalog": {
        "enabled": True,
        "url": "",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `cocso model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.cocso/state.db.
    ...
    "sessions": {
        ...
        # default of ``cocso sessions prune``.
        "retention_days": 90,
    ...
    "updates": {
        ...
        # ``cocso update``.  Backups land in ``<COCSO_HOME>/backups/`` and
        # can be restored with ``cocso import <path>``.  Off by default —
        # on large COCSO_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    # ── Providers (COCSO supports: claude, gpt, codex, mimo, local, custom) ──
    "LM_API_KEY": {
    ...
    # ── Messaging platforms (Discord, Slack, Telegram only) ──
    "TELEGRAM_BOT_TOKEN": {
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'cocso-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote COCSO API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote COCSO API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote COCSO API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.cocso.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older COCSO versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``cocso_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'cocso setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — COCSO won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'cocso doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("COCSO_HOME", "~/.cocso")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("COCSO_TOOL_PROGRESS")
            old_mode = get_env_value("COCSO_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from COCSO_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from COCSO_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("COCSO_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from COCSO_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$COCSO_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_cocso_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `cocso plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: cocso config set <key> <value>")
    ...
    # metadata.cocso.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _cocso_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.cocso/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.cocso/config.yaml.
    ...
    (which change ``COCSO_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_cocso_home()
    config_path = get_config_path()
...
#   xiaomi-mimo  (XIAOMI_API_KEY)      — MiMo
#   custom       (OPENAI_BASE_URL + OPENAI_API_KEY) — any OpenAI-compatible endpoint
#   local        (LM_BASE_URL)         — Ollama / LM Studio / vLLM
#
# fallback_model:
#   provider: anthropic
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.cocso/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.cocso/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real COCSO env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.cocso/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.cocso/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.cocso/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.cocso/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to COCSO — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known COCSO vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.cocso/.env or environment."""
...
def show_config():
    ...
    print(color(f"│              {BRAND_EMOJI} COCSO Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  cocso config edit     # Edit config file", Colors.DIM))
    print(color("  cocso config set <key> <value>", Colors.DIM))
    print(color("  cocso setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: cocso config set <key> <value>")
            print()
            print("Examples:")
            print("  cocso config set model anthropic/claude-sonnet-4")
            print("  cocso config set terminal.backend docker")
            print("  cocso config set ANTHROPIC_API_KEY sk-ant-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'cocso config migrate' to add them")
        ...
        print("  cocso config           Show current configuration")
        print("  cocso config edit      Open config in editor")
        print("  cocso config set <key> <value>   Set a config value")
        print("  cocso config check     Check for missing/outdated config")
        print("  cocso config migrate   Update config with new options")
        print("  cocso config path      Show config file path")
        print("  cocso config env-path  Show .env file path")
        sys.exit(1)
