"""
Single source of truth for provider identity in COCSO Agent.
...
2. **COCSO overlays** — transport type, auth patterns, aggregator flags,
...
from cocso_core.utils import base_url_host_matches, base_url_hostname

logger = logging.getLogger(__name__)


# -- COCSO overlay ----------------------------------------------------------
# COCSO-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class COCSOOverlay:
    """COCSO-specific provider metadata layered on top of models.dev."""
...
COCSO_OVERLAYS: Dict[str, COCSOOverlay] = {
    "openai": COCSOOverlay(
        transport="openai_chat",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "openai-codex": COCSOOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "lmstudio": COCSOOverlay(
        transport="openai_chat",
        auth_type="api_key",
...
ALLOWED_PROVIDER_IDS = {"anthropic", "openai", "openai-codex", "xiaomi", "lmstudio"}
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "cocso", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. COCSO overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + COCSO overlay
    ...
    if canonical not in ALLOWED_PROVIDER_IDS:
        return None
    ...
    overlay = COCSO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + cocso extra
    ...
    if overlay is not None:
        # COCSO-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in COCSO_OVERLAYS
    if provider == "bedrock":
...
def resolve_provider_full(
    ...
    """
    canonical = normalize_provider(name)
    if canonical not in ALLOWED_PROVIDER_IDS and canonical != "custom":
        return None
