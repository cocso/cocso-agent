"""cocso webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    cocso webhook subscribe <name> [options]
    cocso webhook list
    cocso webhook remove <name>
    cocso webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.cocso/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _cocso_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: cocso gateway run
"""
...
def webhook_command(args):
    """Entry point for 'cocso webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: cocso webhook {subscribe|list|remove|test}")
        print("Run 'cocso webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (cocso gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: cocso webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from cocso webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (cocso gateway run)")
