"""
COCSO Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.cocso/ (configs, sessions, logs)
"""
...
from cocso_cli.branding import BRAND_EMOJI, DEFAULT_INSTALL_SCRIPT_URL
...
def remove_path_from_shell_configs():
    """Remove COCSO PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing cocso-agent or cocso PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# COCSO Agent" comment and following line
                if '# COCSO Agent' in line or '# cocso-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('cocso' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing cocso
                if 'cocso' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the cocso wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "cocso",
        Path("/usr/local/bin/cocso"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains cocso_cli reference)
                content = wrapper.read_text()
                if 'cocso_cli' in content or 'cocso-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``cocso gateway run`` processes
    ...
    try:
        from cocso_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from cocso_cli.gateway import get_launchd_plist_path
...
def _is_default_cocso_home(cocso_home: Path) -> bool:
    """Return True when ``cocso_home`` points at the default (non-profile) root."""
    try:
        from cocso_core.cocso_constants import get_default_cocso_root
        return cocso_home.resolve() == get_default_cocso_root().resolve()
    except Exception:
        return False
...
def _discover_named_profiles():
    ...
    default root."""
    try:
        from cocso_cli.profiles import list_profiles
    except Exception:
        return []
    try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its COCSO_HOME directory.

    We shell out to ``cocso -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current COCSO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m cocso_cli.main` so we don't depend on a `cocso`
    #    wrapper that may be half-removed mid-uninstall.
    cocso_invocation = [_sys.executable, "-m", "cocso_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                cocso_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's COCSO_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.cocso/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.cocso/ for future reinstall
    """
    project_root = get_project_root()
    cocso_home = get_cocso_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie COCSO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_cocso_home(cocso_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color(f"│            {BRAND_EMOJI} COCSO Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {cocso_home / 'config.yaml'}")
    print(f"  Secrets: {cocso_home / '.env'}")
    print(f"  Data:    {cocso_home / 'cron/'}, {cocso_home / 'sessions/'}, {cocso_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their COCSO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL COCSO data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the COCSO code but keep your configuration and data.")
    ...
    log_info("Removing cocso command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.cocso/, just remove the cocso-agent subdir
            if cocso_home in project_root.parents or project_root.parent == cocso_home:
    ...
    # 5. Optionally remove ~/.cocso/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile COCSO_HOME dirs live under
        ...
        try:
            if cocso_home.exists():
                shutil.rmtree(cocso_home)
                log_success(f"Removed {cocso_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {cocso_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {cocso_home}")
    ...
    print()
    print(f"Thank you for using COCSO Agent! {BRAND_EMOJI}")
    print()
