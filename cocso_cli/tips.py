...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or cocso -c.",
    ...
    "Ctrl+Z suspends COCSO to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "cocso -c resumes your most recent CLI session. cocso -c \"project name\" resumes by title.",
    "cocso -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "cocso -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "cocso chat -t web,terminal enables only specific toolsets for a focused session.",
    "cocso chat -s github-pr-workflow preloads a skill at launch.",
    "cocso chat -q \"query\" runs a single non-interactive query and exits.",
    "cocso chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "cocso chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "cocso --yolo bypasses all dangerous command approval prompts for the entire session.",
    "cocso chat --source telegram tags the session for filtering in cocso sessions list.",
    "cocso -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "cocso doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "cocso dump outputs a compact setup summary — great for bug reports.",
    "cocso config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "cocso config edit opens config.yaml in your default editor.",
    "cocso config check scans for missing or stale configuration options.",
    "cocso sessions browse opens an interactive session picker with search.",
    "cocso sessions stats shows session counts by platform and database size.",
    "cocso sessions prune --older-than 30 cleans up old sessions.",
    "cocso skills search react --source skills-sh searches the skills.sh public directory.",
    "cocso skills check scans installed hub skills for upstream updates.",
    "cocso skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "cocso skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "cocso mcp add lets you register MCP servers with your own URL or stdio command.",
    "cocso mcp serve runs COCSO itself as an MCP server for other agents.",
    "cocso auth add lets you add multiple API keys for credential pool rotation.",
    "cocso completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "cocso logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "cocso backup creates a zip backup of your entire COCSO home directory.",
    "cocso profile create coder creates an isolated profile that becomes its own command.",
    "cocso profile create work --clone copies your current config and keys to a new profile.",
    "cocso update syncs new bundled skills to ALL profiles automatically.",
    "cocso gateway install sets up COCSO as a system service (systemd/launchd).",
    "cocso memory setup lets you configure an external memory provider via the plugin system.",
    "cocso webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as cocso -w).",
    ...
    "COCSO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call COCSO tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'cocso profile create coder' creates the 'coder' command.",
    "cocso profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, COCSO prints a resume command with session ID and stats.",
    "cocso sessions export backup.jsonl exports all sessions for backup or analysis.",
    "cocso -r SESSION_ID resumes any specific past session by its ID.",
    ...
    # --- Skills ---
    "Bundled skills cover configuration and devops. Install more from GitHub with cocso skills install <repo>.",
    "Every installed skill automatically becomes a slash command — type / to see them all.",
    "cocso skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .cocso/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: cocso cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "COCSO runs on Discord, Slack, and Telegram with a single configuration.",
    "cocso gateway install sets it up as a system service that starts on boot.",
    ...
    "Context files (.cocso.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.cocso/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "COCSO loads project context from .cocso.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "COCSO ships with zero bundled MCP servers — add your own in config.yaml or via cocso mcp add.",
    "Per-server tool filtering: tools.include whitelists and tools.exclude blacklists specific tools.",
    "MCP servers auto-generate toolsets at runtime — cocso tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.cocso/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "cocso chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "cocso plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "COCSO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    ...
    "cocso status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.cocso/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make COCSO your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.cocso/SOUL.md to customize.",
    ...
    "Before a gateway session reset, COCSO auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.cocso/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at COCSO_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "COCSO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in COCSO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 4 priority tiers: cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.cocso/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'cocso profile create ls' would be rejected.",
    "cocso profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "COCSO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via cocso tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.cocso/.env.",
    "Skills from trusted repos get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.cocso/skills/.hub/quarantine/ holds skills pending security review.",
]
