"""
COCSO MCP Server — expose messaging conversations as MCP tools.
...
logger = logging.getLogger("cocso.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using COCSO_HOME."""
    try:
        from cocso_core.cocso_constants import get_cocso_home
        return get_cocso_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("COCSO_HOME", Path.home() / ".cocso")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from cocso_core.cocso_constants import get_cocso_home
        directory_file = get_cocso_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("COCSO_HOME", Path.home() / ".cocso")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            from cocso_core.cocso_constants import get_cocso_home
            db_file = get_cocso_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("COCSO_HOME", Path.home() / ".cocso")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the COCSO MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "cocso",
        instructions=(
            "COCSO Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the COCSO MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
