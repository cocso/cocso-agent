"""``cocso plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.cocso/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    plugins = get_cocso_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from cocso_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_cocso_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_cocso_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_cocso_home()}/.env later)[/dim]")
...
def cmd_install(
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]cocso plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid COCSO plugin."
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `cocso plugins enable {installed_name}` to activate.[/dim]"
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  cocso gateway restart[/dim]")
    console.print()
    ...
    if user_dir.is_dir():
        ...
        for child in user_dir.iterdir():
            ...
            if manifest.get("name") == name:
                return True
    # Bundled: <repo>/plugins/<name>/ (or COCSO_BUNDLED_PLUGINS on Nix).
    from cocso_cli.plugins import get_bundled_plugins_dir
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] cocso plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] cocso plugins")
    console.print("[dim]Enable/disable:[/dim] cocso plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch cocso plugins subcommands."""
