"""
Dump command for cocso CLI.

Outputs a compact, plain-text summary of the user's COCSO setup
...
from cocso_cli.config import get_cocso_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``cocso dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from cocso_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(cocso_home: Path) -> int:
...
def _cron_summary(cocso_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = cocso_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"cocso_home:      {display_cocso_home()}")
    ...
    toolsets = config.get("toolsets", ["cocso-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(cocso_home)}")
    lines.append(f"  skills:             {_count_skills(cocso_home)}")
