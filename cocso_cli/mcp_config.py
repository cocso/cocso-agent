...
COCSO ships with zero bundled MCP servers. Users add each server manually
...
from cocso_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_cocso_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from cocso_cli.cli_output import prompt as _shared_prompt
...
def _interactive_transport_details() -> tuple[Optional[str], Optional[str], List[str]]:
    ...
    print()
    _info("COCSO does not ship with bundled MCP presets.")
...
def cmd_mcp_add(args):
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_cocso_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: cocso mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_cocso_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `cocso mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        _info("COCSO ships with zero bundled MCP servers by default.")
        print()
        _info("Add one with:")
        _info('  cocso mcp add <name> --url <endpoint>')
        _info('  cocso mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `cocso mcp remove` + `cocso mcp add` to reconfigure auth.")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'cocso mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``cocso mcp`` subcommands."""
    action = getattr(args, "mcp_action", None)

    if action == "serve":
        from cocso_cli.mcp_serve import run_mcp_server
    ...
    if handler:
        ...
        _info("cocso mcp serve                              Run as MCP server")
        _info("cocso mcp add <name> --url <endpoint>        Add an MCP server")
        _info("cocso mcp add <name> --command <cmd>         Add a stdio server")
        _info("cocso mcp remove <name>                      Remove a server")
        _info("cocso mcp list                               List servers")
        _info("cocso mcp test <name>                        Test connection")
        _info("cocso mcp configure <name>                   Toggle tools")
        _info("cocso mcp login <name>                       Re-authenticate OAuth")
        print()
