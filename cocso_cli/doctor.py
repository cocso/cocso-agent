"""
Doctor command for cocso CLI.

Diagnoses issues with COCSO Agent setup.
"""
...
from cocso_cli.config import get_project_root, get_cocso_home, get_env_path
from cocso_core.cocso_constants import display_cocso_home

PROJECT_ROOT = get_project_root()
COCSO_HOME = get_cocso_home()
_DHH = display_cocso_home()  # user-facing display path (e.g. ~/.cocso or ~/.cocso/profiles/coder)

# Load environment variables from ~/.cocso/.env so API key checks work
...
from cocso_cli.models import _COCSO_USER_AGENT
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.cocso/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `cocso`.
    os.environ.setdefault("COCSO_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 COCSO Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.cocso/.env (primary location for user config)
    env_path = COCSO_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'cocso setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'cocso setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'cocso setup' to create one")
                issues.append("Run 'cocso setup' to create .env")
    
    # Check ~/.cocso/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = COCSO_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from cocso_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from cocso_cli.providers import (
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'cocso config set model.provider <valid_provider>'"
                    )

            # Warn if model is set to a provider-prefixed name on a provider that doesn't use them
            provider_for_policy = runtime_provider or catalog_provider
            providers_accepting_vendor_slugs = {"custom", "auto", "lmstudio"}
            if (
                default_model
                ...
                issues.append(
                    ...
                    "Either set model.provider to 'custom', or drop the vendor prefix."
                )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    from cocso_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.cocso/.env or run 'cocso setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'cocso setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'cocso config set model.provider <name>'"
                            )
                except Exception:
                    pass
    ...
    if config_path.exists():
        try:
            from cocso_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'cocso setup' to migrate config")
                else:
                    issues.append("Run 'cocso doctor --fix' or 'cocso setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'cocso doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses COCSO' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `cocso auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    cocso_home = COCSO_HOME
    if cocso_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            cocso_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = cocso_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give COCSO a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# COCSO Agent Persona\n\n"
                "<!-- Edit this file to customize how COCSO communicates. -->\n\n"
                "You are COCSO, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = cocso_home / "memories"
    ...
    state_db_path = cocso_home / "state.db"
    ...
    wal_path = cocso_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'cocso doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (cocso bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "cocso"
        ...
        _cmd_link = _cmd_link_dir / "cocso"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(cocso not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/cocso → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/cocso points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/cocso → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/cocso — run 'cocso doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/cocso exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/cocso not found",
                    "(cocso command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/cocso → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/cocso symlink — run 'cocso doctor --fix'")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                issues.append(
                    ...
                    "Fix: run 'cocso config set model.provider <provider>' to switch providers, "
    ...
    _apikey_providers = [
        ...
        ("Xiaomi MiMo", ("XIAOMI_API_KEY",), "https://platform.xiaomimimo.com/openapi/v1/models", "XIAOMI_BASE_URL", True),
        ("LM Studio", ("LM_API_KEY",), None, "LM_BASE_URL", False),
    ]
    ...
    try:
        ...
        from cocso_core.model_tools import check_tool_availability, TOOLSET_REQUIREMENTS
        ...
        if api_disabled:
            issues.append("Run 'cocso setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: cocso skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = COCSO_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: cocso memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: cocso memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from cocso_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "cocso -p" in content:
                            _m = _re.search(r"cocso -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'cocso doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
