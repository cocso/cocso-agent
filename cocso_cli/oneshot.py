...
configured for "cli" in `cocso tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (COCSO_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `cocso chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    ...
    - COCSO_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        from cocso_core.toolsets import validate_toolset
    except Exception as exc:
        return None, f"cocso -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "cocso -z: --toolsets all enables every toolset; "
    ...
    if unresolved:
        try:
            ...
            from cocso_cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        sys.stderr.write(f"cocso -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "cocso -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "cocso -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to COCSO_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            COCSO_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    env_model_early = os.getenv("COCSO_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "cocso -z: --provider requires --model (or COCSO_INFERENCE_MODEL). "
    ...
    os.environ["COCSO_ACCEPT_HOOKS"] = "1"
...
def _run_agent(
    ...
    # Imports are local so they don't run when cocso is invoked for
    ...
    env_model = os.getenv("COCSO_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from cocso_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("COCSO_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                COCSO_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via COCSO_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via COCSO_YOLO_MODE=1
