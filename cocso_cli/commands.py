"""Slash command definitions and autocomplete for the COCSO CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of COCSO config/state", "Session",
    ...
    CommandDef("busy", "Control what Enter does while COCSO is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.cocso/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update COCSO Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`cocso_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/cocso`` subcommand mapping, and
    ...
    """
    try:
        from cocso_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``cocso skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/cocso <verb>`` subcommand.
    ...
    avoidance. ``/cocso`` is always reserved as the first entry so the
    legacy ``/cocso <subcommand>`` form keeps working for anything that
    ...
    # Reserve /cocso as the catch-all top-level command.
    entries.append(("cocso", "Talk to COCSO or run a subcommand", "[subcommand] [args]"))
    seen.add("cocso")
...
def slack_app_manifest(request_url: str = "https://cocso-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /cocso handler.

    Maps both canonical names and aliases so /cocso bg do stuff works
    the same as /cocso background do stuff.

    Plugin-registered slash commands are included so ``/cocso <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from cocso_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
