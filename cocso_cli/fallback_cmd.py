"""
cocso fallback — manage the fallback provider chain.
...
Subcommands:
  cocso fallback [list]   Show the current fallback chain (default when no subcommand)
  cocso fallback add      Pick provider + model via the same picker as `cocso model`,
                           then append the selection to the chain
  cocso fallback remove   Pick an entry to delete from the chain
  cocso fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.cocso/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from cocso_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `cocso model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  cocso fallback add")
        print()
        return
    ...
    from cocso_cli.branding import DEFAULT_REPO_HTTPS_URL
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `cocso model`, then append the selection to the chain."""
    from cocso_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `cocso model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `cocso fallback list` to view, or `cocso fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``cocso fallback [subcommand]``."""
