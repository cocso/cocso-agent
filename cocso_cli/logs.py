"""``cocso logs`` — view and filter COCSO log files.
...
Usage examples::
    ...
    cocso logs                    # last 50 lines of agent.log
    cocso logs -f                 # follow agent.log in real time
    cocso logs errors             # last 50 lines of errors.log
    cocso logs gateway -n 100    # last 100 lines of gateway.log
    cocso logs --level WARNING    # only WARNING+ lines
    cocso logs --session abc123   # filter by session ID substring
    cocso logs --component tools  # only tool-related lines
    cocso logs --since 1h         # lines from the last hour
    cocso logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_cocso_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when COCSO runs — try 'cocso chat' first)")
        sys.exit(1)
    ...
    if component:
        from cocso_core.cocso_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_cocso_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_cocso_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_cocso_home()}/logs/")
        return

    print(f"Log files in {display_cocso_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'cocso chat' to generate logs)")
