"""CLI subcommand: `cocso curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not args.synchronous:
        print("llm pass running in background — check `cocso curator status` later")
    return 0
...
# ---------------------------------------------------------------------------
# argparse wiring (called from cocso_cli.main)
# ---------------------------------------------------------------------------
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by cocso_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="cocso curator")
    register_cli(parser)
