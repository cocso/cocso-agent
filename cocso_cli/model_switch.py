...
- ``cocso_cli.providers``        -- canonical provider identity + overlays
- ``cocso_cli.model_normalize``  -- per-provider name formatting
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from cocso_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'cocso model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'cocso doctor' — config issues detected:"
    ...
    # Mirrors the same logic in cocso_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from cocso_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from cocso_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by cocso provider ID
    ...
    # --- 1. Check COCSO-mapped providers ---
    for cocso_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(cocso_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (COCSO_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from cocso_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and cocso_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a COCSO release.
        model_ids = curated.get(cocso_id, [])
        if cocso_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(cocso_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check COCSO-only providers (openai-codex) ---
    from cocso_cli.providers import COCSO_OVERLAYS
    from cocso_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → COCSO provider ID.
    # COCSO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use COCSO IDs ("copilot").
    _mdev_to_cocso = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in COCSO_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve COCSO slug — e.g. "github-copilot" → "copilot"
        cocso_slug = _mdev_to_cocso.get(pid, pid)
        if cocso_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or cocso_slug in providers_store
                    or pid in pool_store or cocso_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(cocso_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", cocso_slug, exc)
        ...
        # configured.
        if not has_creds and cocso_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_cocso_oauth_credentials,
                )
                cocso_creds = read_cocso_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (cocso_creds and cocso_creds.get("accessToken")) or \
        ...
        if cocso_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(cocso_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(cocso_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(cocso_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by COCSO slug, fall back to overlay key
            model_ids = curated.get(cocso_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if cocso_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(cocso_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": cocso_slug,
            "name": get_label(cocso_slug),
            "is_current": cocso_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "cocso",
        ...
        seen_slugs.add(cocso_slug.lower())
        _record_builtin_endpoint(cocso_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or COCSO_OVERLAYS (keeps /model in sync
    # with `cocso model`).
    try:
        from cocso_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is COCSO's canonical write key (matches
            ...
            # COCSO writes ``models:`` as a dict keyed by model id
            # (see cocso_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # COCSO's own writer uses; a hyphen variant is accepted
            ...
            # active model. COCSO's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, cocso_cli/config.py) already consume that dict.
