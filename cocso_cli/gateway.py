"""
Gateway subcommand for cocso CLI.

Handles: cocso gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_cocso_home is imported lazily at call sites to avoid ImportError
# when cocso_constants is cached from a pre-update version during `cocso update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "cocso-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "cocso_cli.main gateway",
        "cocso_cli.main --profile",
        "cocso_cli.main -p",
        "cocso_cli/main.py gateway",
        "cocso_cli/main.py --profile",
        "cocso_cli/main.py -p",
        "cocso gateway",
        "gateway/run.py",
    ]
    current_home = str(get_cocso_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"COCSO_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "COCSO_HOME=" in command and f"COCSO_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``cocso update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            COCSO profile are returned.
    """
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}cocso gateway status\n"
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `cocso gateway`")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (COCSO_HOME-scoped).
...
from cocso_core.cocso_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "cocso-gateway"
SERVICE_DESCRIPTION = "COCSO Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current COCSO_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (COCSO_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from cocso_core.cocso_constants import get_default_cocso_root
    home = get_cocso_home().resolve()
    default = get_default_cocso_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary COCSO_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(cocso_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when COCSO_HOME is a named profile.

    For ``~/.cocso/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        cocso_home: Optional explicit COCSO_HOME path. Defaults to the current
            ``get_cocso_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from cocso_core.cocso_constants import get_default_cocso_root
    home = Path(cocso_home or str(get_cocso_home())).resolve()
    default = get_default_cocso_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this COCSO_HOME.

    Default ``~/.cocso`` returns ``cocso-gateway`` (backward compatible).
    Profile ``~/.cocso/profiles/coder`` returns ``cocso-gateway-coder``.
    Any other COCSO_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older COCSO installs that predate the
# cocso-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (cocso-gateway-*.service) and unrelated third-party
# "cocso" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("cocso.service",)
...
def _find_legacy_cocso_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy COCSO gateway units.

    Detects unit files installed by older COCSO versions that used a
    different service name (e.g. ``cocso.service`` before the rename to
    ``cocso-gateway.service``). When both a legacy unit and the current
    ``cocso-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``cocso-gateway-coder.service`` and unrelated third-party
      ``cocso-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``cocso.service`` running an unrelated
...
def has_legacy_cocso_units() -> bool:
    """Return True when any legacy COCSO gateway unit files exist."""
    return bool(_find_legacy_cocso_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy COCSO gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_cocso_units()
    if not legacy:
        return
    print_warning("Legacy COCSO gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current cocso-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    cocso gateway migrate-legacy")


def remove_legacy_cocso_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy COCSO gateway unit files.

    Iterates over whatever ``_find_legacy_cocso_units()`` returns — which is
    ...
    legacy = _find_legacy_cocso_units()
    if not legacy:
        print("No legacy COCSO gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy COCSO gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: cocso gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo cocso gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    cocso gateway uninstall")
    print_info("    sudo cocso gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so COCSO can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo cocso gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo cocso gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo cocso gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode COCSO often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.cocso`` → ``ai.cocso.gateway.plist`` (backward compatible).
    Profile ``~/.cocso/profiles/coder`` → ``ai.cocso.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.cocso.gateway-{suffix}" if suffix else "ai.cocso.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.cocso/cocso-agent  -> /home/alice/.cocso/cocso-agent
      /opt/cocso                 -> /opt/cocso  (kept as-is)
...
def _cocso_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current COCSO_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_cocso_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.cocso                    → /home/alice/.cocso
      /root/.cocso/profiles/coder     → /home/alice/.cocso/profiles/coder
      /opt/custom-cocso               → /opt/custom-cocso  (kept as-is)
    """
    current_cocso = get_cocso_home().resolve()
    current_default = (Path.home() / ".cocso").resolve()
    target_default = Path(target_home_dir) / ".cocso"

    # Default ~/.cocso → remap to target user's default
    if current_cocso == current_default:
        return str(target_default)

    # Profile or subdir of ~/.cocso → preserve the relative structure
    try:
        relative = current_cocso.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.cocso) — keep as-is
        return str(current_cocso)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        cocso_home = _cocso_home_for_target_user(home_dir)
        profile_arg = _profile_arg(cocso_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m cocso_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="COCSO_HOME={cocso_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    cocso_home = str(get_cocso_home().resolve())
    profile_arg = _profile_arg(cocso_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current COCSO install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (cocso.service from pre-rename installs)
    # before installing the new cocso-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}cocso gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}cocso gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `cocso gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}cocso gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}cocso gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}cocso gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}cocso gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.cocso.gateway-{suffix}" if suffix else "ai.cocso.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current COCSO install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  cocso gateway status             # Check status")
...
def launchd_stop():
    ...
    # `cocso gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current COCSO install")
    else:
        print("⚠ Service definition is stale relative to the current COCSO install")
        print("  Run: cocso gateway start")
    ...
    if deep:
        log_file = get_cocso_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print(f"│           {BRAND_EMOJI} COCSO Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
def _all_platforms() -> list[dict]:
    ...
    ``cocso setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.cocso/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_cocso_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'cocso pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: cocso pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'cocso gateway setup'")
            continue
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``cocso_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from cocso_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.cocso/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.cocso/.env: {', '.join(required)}")
    else:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: cocso gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  cocso gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: cocso gateway run")
                print_info("  For persistence:   tmux new -s cocso 'cocso gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: cocso gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup cocso gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: cocso gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'cocso gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: cocso gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: cocso gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s cocso 'cocso gateway run'")
                print()
            ...
            print()
            print("  cocso gateway run                              # direct foreground")
            print("  tmux new -s cocso 'cocso gateway run'         # persistent via tmux")
            print("  nohup cocso gateway run > ~/.cocso/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: cocso gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: cocso gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: cocso gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: cocso gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: cocso gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  cocso gateway install")
                    print("  sudo cocso gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  cocso gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup cocso gateway run > ~/.cocso/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s cocso 'cocso gateway run'         # persistent via tmux")
                    print("  nohup cocso gateway run > ~/.cocso/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  cocso gateway install  # Install as user service")
                    print("  sudo cocso gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy COCSO gateway unit files from
        # pre-rename installs (e.g. cocso.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_cocso_units(interactive=not yes, dry_run=dry_run)