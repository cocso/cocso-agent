...
Pure display functions with no COCSOCLI state dependency.
"""
...
def _skin_color(key: str, fallback: str = "") -> str:
    ...
    from cocso_cli.branding import default_color
...
from cocso_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
from cocso_cli.branding import BANNER_LOGO, BANNER_HERO_ART
...
# (e.g. nix-built cocso — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

from cocso_cli.branding import DEFAULT_REPO_URL as _UPSTREAM_REPO_URL
...
def check_for_updates() -> Optional[int]:
    """Check whether a COCSO update is available.

    Two paths: if ``COCSO_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = cocso_home / ".update_check"
    embedded_rev = os.environ.get("COCSO_REVISION") or None
    ...
    if embedded_rev:
        ...
        repo_dir = cocso_home / "cocso-agent"
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active COCSO git checkout, or None if this isn't a git install."""
    cocso_home = get_cocso_home()
    repo_dir = cocso_home / "cocso-agent"
    if not (repo_dir / ".git").exists():
        repo_dir = Path(__file__).parent.parent.resolve()
...
from cocso_cli.branding import DEFAULT_RELEASE_URL_BASE as _RELEASE_URL_BASE
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    COCSO checkout. Cached per-process. Release URL always points at the
    canonical cocso/cocso-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    agent_name = default_branding("agent_name", "COCSO Agent")
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    if get_toolset_for_tool is None:
        from cocso_core.model_tools import get_toolset_for_tool
    ...
    from cocso_cli.skin_engine import banner_layout_enabled
    ...
    if show_custom:
        try:
            ...
            custom_lines = []
        if not custom_lines:
            from cocso_cli.branding import DEFAULT_BANNER_CUSTOM_LINES
            custom_lines = list(DEFAULT_BANNER_CUSTOM_LINES)
        try:
            from cocso_cli.branding import DEFAULT_BANNER_CUSTOM_POSITION
    ...
    if show_update:
        try:
            ...
            if behind is not None and behind != 0:
                from cocso_cli.config import get_managed_update_command, recommended_update_command
                if behind > 0:
                    ...
                    # UPDATE_AVAILABLE_NO_COUNT: nix-built cocso; we know an update
