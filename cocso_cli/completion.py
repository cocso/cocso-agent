"""Shell completion script generation for cocso CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_cocso_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# COCSO Agent bash completion
# Add to ~/.bashrc:
#   eval "$(cocso completion bash)"
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef cocso
# COCSO Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(cocso completion zsh)"
...
_cocso() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_cocso_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'cocso command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# COCSO Agent fish completion",
        "# Add to your config:",
        "#   cocso completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __cocso_profiles",
        "    echo default",
        "    if test -d $HOME/.cocso/profiles",
        "        ls $HOME/.cocso/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c cocso -f -s p -l profile"
        " -d 'Profile name' -xa '(__cocso_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__cocso_profiles)' -d 'Profile name'"
                )
