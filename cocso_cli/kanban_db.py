...
The board lives at ``$COCSO_HOME/kanban.db`` (profile-agnostic on purpose:
...
coding workloads.  See ``docs/cocso-kanban-v1-spec.pdf`` for the full
...
def kanban_db_path() -> Path:
    """Return the path to ``kanban.db`` inside the active COCSO_HOME."""
    from cocso_core.cocso_constants import get_cocso_home
    return get_cocso_home() / "kanban.db"


def workspaces_root() -> Path:
    """Return the directory under which ``scratch`` workspaces are created."""
    from cocso_core.cocso_constants import get_cocso_home
    return get_cocso_home() / "kanban" / "workspaces"
...
def init_db(db_path: Optional[Path] = None) -> Path:
    ...
    Kept as a public entry point so CLI ``cocso kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``cocso --skills ...`` alongside the built-in
    ...
    # `cocso --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``cocso kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``cocso kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``cocso kanban complete <id>``) works without requiring
...
def resolve_workspace(task: Task) -> Path:
    ...
    - ``scratch``: a fresh dir under ``$COCSO_HOME/kanban/workspaces/<id>/``.
    - ``dir:<path>``: the path stored in ``workspace_path``.  Created
      ...
      root.  Users who want a COCSO_HOME-relative workspace should
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``cocso kanban
...
def _default_spawn(task: Task, workspace: str) -> Optional[int]:
    """Fire-and-forget ``cocso -p <profile> chat -q ...`` subprocess.
    ...
    env = dict(os.environ)
    if task.tenant:
        env["COCSO_TENANT"] = task.tenant
    env["COCSO_KANBAN_TASK"] = task.id
    env["COCSO_KANBAN_WORKSPACE"] = workspace
    # COCSO_PROFILE is the author the kanban_comment tool defaults to.
    # `cocso -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["COCSO_PROFILE"] = task.assignee
    ...
    # Redirect output to a per-task log under COCSO_HOME/kanban/logs/.
    from cocso_core.cocso_constants import get_cocso_home
    log_dir = get_cocso_home() / "kanban" / "logs"
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`cocso` executable not found on PATH. "
            "Install Cocso Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``cocso kanban daemon`` is systemd-friendly.
...
def worker_log_path(task_id: str) -> Path:
    ...
    from cocso_core.cocso_constants import get_cocso_home
    return get_cocso_home() / "kanban" / "logs" / f"{task_id}.log"
...
def list_profiles_on_disk() -> list[str]:
    ...
    Reads ``~/.cocso/profiles/`` directly so this module has no import
    dependency on ``cocso_cli.profiles`` (which pulls in a large chunk
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``cocso kanban assignees`` for the terminal.
