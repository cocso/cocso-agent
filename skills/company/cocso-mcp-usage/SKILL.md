...
### 비슷한 tool 중에서 선택 (강한 규칙)

이름이 비슷한 tool 여러 개 있으면 (`*_get_*` vs `*_lookup_*` vs `*_search_*`):

1. **인벤토리 description 정독** — 한 줄 차이 자세히 읽기.
2. **파라미터 비교** — required field 가 사용자 발화에서 추출 가능한지.
3. **차이가 모호하면 사용자에게 물어보기**: "두 tool 후보 있습니다. (a) ○○ — 설명. (b) ×× — 설명. 어느 쪽을 쓸까요?"
4. **추측 호출 금지** — confidence 낮으면 사용자 확인 우선.

### 응답 결과가 ambiguous할 때

검색·조회 결과가 여러 개 (예: `service_search_medicines("타이레놀")` → 5개 약품):

- 결과 1개 → 즉시 사용
- 결과 2~5개 → **사용자에게 목록 보여주고 선택 요청**:
  > "타이레놀 검색 결과 5개입니다. 어느 제품 EDI를 쓰시겠어요?
  > 1. 타이레놀 정 500mg (EDI: 645000570)
  > 2. 타이레놀 ER 정 650mg (EDI: 645000680)
  > ..."
- 결과 0개 → "검색 결과 없음. 다른 키워드?" 또는 "수동 입력 필요" 알림
- 결과 50+ → search_word 보강해 좁히기 권장

**절대 임의 선택 금지** — 잘못된 EDI = 잘못된 정산.

### 실패 / Retry 정책

| 에러 | 1회 재시도 | 2회 재시도 | 그 후 |
|---|---|---|---|
| `tool not found` | 인벤토리 재호출 후 재시도 | (재호출도 실패면) | 사용자에게 "tool 미존재" 알림 |
| `invalid params` / schema mismatch | 인벤토리 재호출 → 새 schema 로 인자 재매핑 → 재시도 | — | 사용자 확인 |
| `timeout` (5xx, 30s+) | 5초 wait 후 재시도 | — | 사용자 확인 + `cocso doctor` 안내 |
| `401` / `403` | 재시도 안 함 | — | "MCP 키 만료/권한 부족" + 운영팀 안내 |
| `404` (data not found) | 재시도 안 함 | — | "데이터 없음" 사용자 알림 |
| `5xx` (서버 오류) | 5초 wait 후 1회 재시도 | — | 사용자 확인 + 운영팀 알림 |

**무한 retry loop 절대 금지** — 1~2회 후 사용자에게 결정권 넘김.
