"""Shared fixtures for the cocso-agent test suite.
...
2. **Isolated COCSO_HOME.** COCSO_HOME points to a per-test tempdir so
   code reading ``~/.cocso/*`` via ``get_cocso_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".cocso"`` instead of the canonical
   ``get_cocso_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No COCSO_SESSION_* inheritance** — the agent's current gateway
...
# COCSO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_COCSO_BEHAVIORAL_VARS = frozenset({
    "COCSO_YOLO_MODE",
    "COCSO_INTERACTIVE",
    "COCSO_QUIET",
    "COCSO_TOOL_PROGRESS",
    "COCSO_TOOL_PROGRESS_MODE",
    "COCSO_MAX_ITERATIONS",
    "COCSO_SESSION_PLATFORM",
    "COCSO_SESSION_CHAT_ID",
    "COCSO_SESSION_CHAT_NAME",
    "COCSO_SESSION_THREAD_ID",
    "COCSO_SESSION_SOURCE",
    "COCSO_SESSION_KEY",
    "COCSO_GATEWAY_SESSION",
    "COCSO_PLATFORM",
    "COCSO_MODEL",
    "COCSO_INFERENCE_MODEL",
    "COCSO_INFERENCE_PROVIDER",
    "COCSO_TUI_PROVIDER",
    "COCSO_MANAGED",
    "COCSO_DEV",
    "COCSO_CONTAINER",
    ...
    "COCSO_TIMEZONE",
    "COCSO_REDACT_SECRETS",
    "COCSO_BACKGROUND_NOTIFICATIONS",
    "COCSO_EXEC_ASK",
    "COCSO_HOME_MODE",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects COCSO_HOME to a per-test tempdir so code that reads
    ``~/.cocso/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral COCSO_* vars that could change test semantics.
    for name in _COCSO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect COCSO_HOME to a per-test tempdir. Code that reads
    #    ``~/.cocso/*`` via ``get_cocso_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.cocso/*`` via
    #    ``Path.home() / ".cocso"`` instead of ``get_cocso_home()``
    #    is a bug to fix at the callsite.
    fake_cocso_home = tmp_path / "cocso_test"
    fake_cocso_home.mkdir()
    (fake_cocso_home / "sessions").mkdir()
    (fake_cocso_home / "cron").mkdir()
    (fake_cocso_home / "memories").mkdir()
    (fake_cocso_home / "skills").mkdir()
    monkeypatch.setenv("COCSO_HOME", str(fake_cocso_home))
    ...
    #    ~/.cocso/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import cocso_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_cocso_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "cocso_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal cocso config dict suitable for unit tests."""
    return {
...
def _reset_tool_registry_caches():
    ...
    its result (see cocso_core/model_tools.py). Both are keyed on state
    that tests routinely mutate (env vars, registry._generation,
    config.yaml mtime) — but a stale result from test A can still be
    served to test B because 30 s covers the entire suite, and xdist
    worker reuse means one test's cache lands in another's process.
    Clearing before every test keeps hermetic behavior.
    """
    try:
        ...
        pass
    try:
        from cocso_core.model_tools import _clear_tool_defs_cache
