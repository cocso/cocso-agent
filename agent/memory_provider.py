...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - cocso_home (str): The active COCSO_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.cocso``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "cocso").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'cocso memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], cocso_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'cocso memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``cocso_home`` is the active COCSO_HOME directory path.
