...
# `export COCSO_REDACT_SECRETS=true`) cannot enable/disable redaction
# mid-session.  OFF by default — user must opt in via
# `security.redact_secrets: true` in config.yaml (bridged to this env var
# in cocso_cli/main.py and gateway/run.py) or `COCSO_REDACT_SECRETS=true`
# in ~/.cocso/.env.
_REDACT_ENABLED = os.getenv("COCSO_REDACT_SECRETS", "").lower() in ("1", "true", "yes", "on")
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across COCSO — used by
    ``cocso config``, ``cocso status``, ``cocso dump``, and anywhere
