...
def _state_file() -> Path:
    return get_cocso_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.cocso/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    "You are running as COCSO' background skill CURATOR. This is an "
    ...
    "into ~/.cocso/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: cocso-config-*, cocso-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.cocso/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.cocso/logs/curator/``)
    ...
    authored skill data in ``~/.cocso/skills/``.
    """
    return get_cocso_home() / "logs" / "curator"
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if archived:
        ...
        lines.append("_Archived skills are at `~/.cocso/skills/.archive/`. "
                     "Restore any via `cocso curator restore <name>`._\n")
    ...
    lines.append("- Restore an archived skill: `cocso curator restore <name>`")
    lines.append("- All archives live under `~/.cocso/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    def _llm_pass():
        ...
        # recorded in state so `cocso curator status` can point at it.
        try:
