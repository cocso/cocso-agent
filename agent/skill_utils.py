...
from cocso_core.cocso_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``COCSO_PLATFORM`` or
            ``COCSO_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.cocso/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against COCSO_HOME, not cwd
        if not p.is_absolute():
            p = (cocso_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.cocso/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    cocso = metadata.get("cocso") or {}
    if not isinstance(cocso, dict):
        cocso = {}
    return {
        "fallback_for_toolsets": cocso.get("fallback_for_toolsets", []),
        "requires_toolsets": cocso.get("requires_toolsets", []),
        "fallback_for_tools": cocso.get("fallback_for_tools", []),
        "requires_tools": cocso.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    cocso = metadata.get("cocso")
    if not isinstance(cocso, dict):
        return []
    raw = cocso.get("config")
    if not raw:
        return []
