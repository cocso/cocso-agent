"""Unified removal contract for every credential source COCSO reads from.

COCSO seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.cocso/.env
    claude_code   — ~/.claude/.credentials.json
    cocso_pkce   — ~/.cocso/.anthropic_oauth.json
    ...
    manual        — user ran `cocso auth add`
...
unify here is **removal**:
    ...
    ``cocso auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), cocso_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.cocso/.env  → clear it
    ...
    """
    from cocso_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to COCSO.",
            f"  The pool entry is now suppressed — COCSO will ignore "
            f"{env_var} until you run `cocso auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so COCSO stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `cocso auth add anthropic` to re-enable if needed.",
    ])


def _remove_cocso_pkce(provider: str, removed) -> RemovalResult:
    """~/.cocso/.anthropic_oauth.json is ours — delete it outright."""
    from cocso_core.cocso_constants import get_cocso_home

    result = RemovalResult()
    oauth_file = get_cocso_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared COCSO Anthropic OAuth credentials")
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the COCSO auth store is not enough — _seed_from_singletons() would
    ...
    via ``cocso auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from cocso_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `cocso auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `cocso auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    COCSO stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `cocso auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="cocso_pkce",
        remove_fn=_remove_cocso_pkce,
        description="~/.cocso/.anthropic_oauth.json",
    ))
    register(RemovalStep(
