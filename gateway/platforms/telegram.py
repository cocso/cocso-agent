...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("COCSO_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = float(os.getenv("COCSO_TELEGRAM_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("COCSO_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "(possibly OpenClaw or another COCSO instance). "
            "COCSO stopped Telegram polling after %d retries. "
            "Only one poller can run per token — stop the other process "
            "and restart with 'cocso start'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("COCSO_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("COCSO_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("COCSO_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("COCSO_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("COCSO_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("COCSO_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in ("1", "true", "yes", "on"))
            ...
            if webhook_url:
                ...
                if not webhook_secret:
                    raise RuntimeError(
                        ...
                        "https://github.com/cocso/cocso-agent/"
            ...
            try:
                ...
                from cocso_cli.commands import telegram_menu_commands
    ...
    async def send_update_prompt(
        ...
        try:
            ...
            text = f"{BRAND_EMOJI} *Update needs your input:*\n\n{prompt}{default_hint}"
    ...
    async def _handle_callback_query(
        ...
        try:
            await query.edit_message_text(
                text=f"{BRAND_EMOJI} Update prompt answered: *{label}*",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=None,
            )
        except Exception:
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@cocso_bot.example" are not mentions
