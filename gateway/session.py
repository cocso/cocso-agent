...
from .whatsapp_identity import (
    ...
    normalize_whatsapp_identifier,  # noqa: F401 - compatibility shim
)
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `cocso tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `cocso tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_cocso_home()}/cron/output/)"
    )
