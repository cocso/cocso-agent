...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_cocso_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.cocso/config.yaml (primary user-facing config)
    3. ~/.cocso/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_cocso_home()
    gw_data: dict = {}
...
def _validate_gateway_config(config: "GatewayConfig") -> None:
    ...
    try:
        from cocso_cli.auth import has_usable_secret
    except ImportError:
