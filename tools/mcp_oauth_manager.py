...
`mcp_tool.py`, and `cocso_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# COCSOMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_cocso_provider_class() -> Optional[type]:
    ...
    class COCSOMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._cocso_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`COCSOTokenStorage` persisting an absolute
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._cocso_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._cocso_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return COCSOMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_COCSO_PROVIDER_CLS: Optional[type] = _make_cocso_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`COCSOMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _COCSO_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = COCSOTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``cocso mcp remove <name>`` and (indirectly) by
        ``cocso mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
