"""Central registry for all cocso-agent tools.
...
Import chain (circular-import safe):
    tools/registry.py  (no imports from cocso_core.model_tools or tool files)
           ^
...
# probe external state (Docker daemon, playwright binary availability).
# For a long-lived CLI or gateway process, calling them on
# every get_definitions() is pure waste — external state changes on human
# timescales. Cache results for ~30 s so env-var flips via ``cocso tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``cocso tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        requirements probes docker, browser checks probe playwright, etc.);
        TTL chosen so env-var changes (``cocso tools enable foo``) still
        take effect in near-real-time without forcing a full cache flush
        on every call.
        """
        result = []
