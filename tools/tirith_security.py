...
it is automatically downloaded from GitHub releases to $COCSO_HOME/bin/tirith.
...
def _get_cocso_home() -> str:
    """Return the COCSO home directory, respecting COCSO_HOME env var."""
    return str(get_cocso_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_cocso_home(), ".tirith-install-failed")
...
def _cocso_bin_dir() -> str:
    """Return $COCSO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_cocso_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $COCSO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_cocso_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $COCSO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $COCSO_HOME/bin/tirith
    ...
    cocso_bin = os.path.join(_cocso_bin_dir(), "tirith")
    if os.path.isfile(cocso_bin) and os.access(cocso_bin, os.X_OK):
        _resolved_path = cocso_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return cocso_bin
