...
Architecture:
    ~/.cocso/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        COCSO_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_cocso_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("COCSO_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(shadow_repo: Path, working_dir: str) -> dict:
    ...
    The shadow repo is internal COCSO infrastructure — it must NOT inherit
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "cocso@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "COCSO Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "COCSO_WORKDIR").write_text(
...
#   * orphan:  the ``COCSO_WORKDIR`` path no longer exists on disk
...
def _read_workdir_marker(shadow_repo: Path) -> Optional[str]:
    """Read ``COCSO_WORKDIR`` from a shadow repo, or None if missing/unreadable."""
    try:
        return (shadow_repo / "COCSO_WORKDIR").read_text(encoding="utf-8").strip()
...
def prune_checkpoints(
    ...
    * ``delete_orphans=True`` and its ``COCSO_WORKDIR`` path no longer
...
def maybe_auto_prune_checkpoints(
    ...
    bounded regardless of how many times cocso is invoked per day.
