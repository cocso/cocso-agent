...
from cocso_core.toolsets import TOOLSETS
from tools import file_state
from tools.terminal_tool import set_approval_callback as _set_subagent_approval_cb
from cocso_core.utils import base_url_hostname, is_truthy_value
...
# (cocso-* prefixed), and scenario toolsets.
#
...
def _build_child_agent(
    ...
    if parent_enabled is not None:
        ...
        from cocso_core import model_tools
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``$COCSO_HOME/logs/subagent-<sid>-<ts>.log``
...
def delegate_task(
    ...
    from cocso_core import model_tools as _model_tools
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'cocso auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (cocso_cli/config.py load_config()) so that
