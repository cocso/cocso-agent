#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the COCSO Skills Hub.
...
Used by cocso_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            cocso_meta = metadata.get("cocso", {})
            if isinstance(cocso_meta, dict):
                tags = cocso_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by COCSO) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.cocso/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                cocso_meta = meta_block.get("cocso", {})
                if isinstance(cocso_meta, dict):
                    tags = cocso_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# COCSO centralized index source
# ---------------------------------------------------------------------------

COCSO_INDEX_URL = os.environ.get("COCSO_SKILLS_INDEX_URL", "").strip()
COCSO_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "cocso-index.json"
COCSO_INDEX_TTL = 6 * 3600  # 6 hours


def _load_cocso_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    Disabled when ``COCSO_SKILLS_INDEX_URL`` is unset. Otherwise fetches
    from the configured URL (a JSON manifest you host yourself) and caches
    locally for COCSO_INDEX_TTL seconds.
    """
    if not COCSO_INDEX_URL:
        return _load_stale_index_cache()

    # Check local cache
    if COCSO_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - COCSO_INDEX_CACHE_FILE.stat().st_mtime
            if age < COCSO_INDEX_TTL:
                return json.loads(COCSO_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from configured index URL
    try:
        resp = httpx.get(COCSO_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("COCSO index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("COCSO index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        COCSO_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        COCSO_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class COCSOIndexSource(SkillSource):
    """Skill source backed by the centralized COCSO Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "cocso-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "cocso-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        COCSOIndexSource(auth=auth), # Centralized index (search + resolved install paths)
