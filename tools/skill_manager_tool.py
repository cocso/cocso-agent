...
~/.cocso/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `cocso config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.cocso/skills/ (single source of truth)
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    ``cocso curator unpin <name>`` (or edit it directly by hand).  This
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`cocso curator unpin {name}` if they want the change."
            )
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.cocso/skills/) first, then any
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_cocso_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "pointing the user to `cocso curator unpin <name>`. Don't try to route "
