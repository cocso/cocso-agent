...
This module provides the glue:
    - ``COCSOTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "COCSO Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses COCSO_HOME so each profile gets its own OAuth tokens.
    Layout: ``COCSO_HOME/mcp-tokens/``
    """
    try:
        from cocso_core.cocso_constants import get_cocso_home
        base = Path(get_cocso_home())
    except ImportError:
        base = Path(os.environ.get("COCSO_HOME", str(Path.home() / ".cocso")))
...
# ---------------------------------------------------------------------------
# COCSOTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class COCSOTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        COCSO_HOME/mcp-tokens/<server_name>.json         -- tokens
        COCSO_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # COCSO records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to COCSO.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "COCSO Agent")
    scope = cfg.get("scope")
