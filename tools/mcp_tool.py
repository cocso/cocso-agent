...
discovers their tools, and registers them into the cocso-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.cocso/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.cocso/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            cocso_home = os.path.expanduser(
                os.getenv(
                    "COCSO_HOME", os.path.join(os.path.expanduser("~"), ".cocso")
                )
            )
            candidates = [
                os.path.join(cocso_home, "node", "bin", resolved_command),
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `cocso mcp login {server_name}` (or delete the tokens "
            f"file under ~/.cocso/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the COCSO config file.
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves COCSO's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the COCSO registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``cocso tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the COCSO registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple cocso
