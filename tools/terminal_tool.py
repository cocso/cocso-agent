...
Executes shell commands in local, Docker, or SSH environments.
...
# display_cocso_home imported lazily at call site (stale-module safety during cocso update)
...
def _get_scratch_dir() -> Path:
    """Return the per-user scratch directory for sandbox state."""
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("COCSO_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("COCSO_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("COCSO_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (COCSO_INTERACTIVE=1).
    ...
    try:
        os.environ["COCSO_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "COCSO_SPINNER_PAUSE" in os.environ:
            del os.environ["COCSO_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (COCSO_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and os.getenv("COCSO_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so COCSO can track lifecycle and output.
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.cocso/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    # remote home, docker starts in the container's default root-like cwd.
    if env_type == "local":
    ...
    if env_type == "docker" and mount_docker_cwd:
        ...
        if (is_host_path or is_relative) and cwd != default_cwd:
            logger.info("Ignoring TERMINAL_CWD=%r for docker backend "
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
                        ...
                        task_id: str = "default",
                        host_cwd: str = None):
    """Create an execution environment. COCSO supports local / docker / ssh."""
...
def cleanup_all_environments():
    ...
    import glob
    for path in glob.glob(str(scratch_dir / "cocso-*")):
        try:
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(command):
        return (
            ...
            "Use terminal(background=true) so COCSO can track the process, then run "
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("COCSO_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("COCSO_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("COCSO_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("COCSO_SESSION_USER_ID", "")
                        _gw_user_name = _gse("COCSO_SESSION_USER_NAME", "")
            ...
            try:
                from cocso_cli.plugins import invoke_hook
