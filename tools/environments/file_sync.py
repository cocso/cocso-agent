...
_FORCE_SYNC_ENV = "COCSO_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.cocso") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.cocso to *container_base* because the remote
    user's home may differ (e.g. /home/user).
    """
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``COCSO_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, cocso_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.cocso/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="cocso-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.cocso/skills/a.md`` →
        ``~/.cocso/skills/a.md``, a new remote file at
        ``/root/.cocso/skills/b.md`` maps to ``~/.cocso/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
