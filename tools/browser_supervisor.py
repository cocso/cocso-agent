...
One ``CDPSupervisor`` runs per COCSO ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "cocso-dialog-bridge.invalid"
...
(() => {
  if (window.__cocsoDialogBridgeInstalled) return;
  window.__cocsoDialogBridgeInstalled = true;
  const ENDPOINT = "http://cocso-dialog-bridge.invalid/";
