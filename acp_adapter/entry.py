"""CLI entry point for the cocso-agent ACP adapter.

Loads environment variables from ``~/.cocso/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from COCSO_HOME (default ``~/.cocso``)."""
    from cocso_cli.env_loader import load_cocso_dotenv

    cocso_home = get_cocso_home()
    loaded = load_cocso_dotenv(cocso_home=cocso_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", cocso_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting cocso-agent ACP adapter")

    # Ensure the project root is on sys.path so ``from cocso_cli.run_agent import AIAgent`` works
