"""ACP agent server — exposes Cocso Agent via the Agent Client Protocol."""
...
try:
    from cocso_cli import __version__ as HERMES_VERSION
except Exception:
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Cocso/OpenAI-compatible user content payload."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Cocso AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Cocso version",
        },
    )
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from cocso_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from cocso_cli.models import detect_provider_for_model, parse_model_input
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            from cocso_core.model_tools import get_tool_definitions
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Cocso using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="cocso-agent", version=HERMES_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Cocso' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        restoring server-side state makes Cocso remember context, but leaves the
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Cocso on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # not the executor's. Also set COCSO_INTERACTIVE so approval.py
        ...
        def _run_agent() -> dict:
            ...
            previous_interactive = os.environ.get("COCSO_INTERACTIVE")
            ...
            try:
                ...
                # Restore COCSO_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("COCSO_INTERACTIVE", None)
                else:
                    os.environ["COCSO_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Cocso Agent v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Cocso has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
